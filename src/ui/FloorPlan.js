...
export function createFloorPlan(config) {
  ...
  let svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 850" role="img" aria-label="Workshop 14.435 by 5.406 metres with the restored earlier bathroom, Other 1, Other 2 and Other 3 layout" font-family="Segoe UI,Arial,sans-serif">';
  ...
  for (const area of [plan.lobby, plan.balcony]) svg += rect(area.minX, area.minZ, area.maxX-area.minX, area.maxZ-area.minZ, '#e2e7d8', '#a4b191');
  ...
  const cabinet = config.objects.windowCabinet;
  for (const [item, width, depth] of [[config.objects.shelf, .48, 1.4], [cabinet, cabinet.depth, cabinet.width]]) svg += rect(item.position[0] - width / 2, item.position[2] - depth / 2, width, depth, '#bea67d', '#907950');
  for (let i = 1; i < cabinet.doorCount; i++) {
    const z = cabinet.position[2] - cabinet.width / 2 + i * cabinet.width / cabinet.doorCount;
    svg += line([cabinet.position[0] - cabinet.depth / 2, z], [cabinet.position[0] + cabinet.depth / 2, z], '#907950', 1);
  }
  ...
  for (const room of plan.rooms) {
    ...
    svg += text(x, labelZ, room.label, 8) + (room.dimensionLabel ? text(x, labelZ + .32, room.dimensionLabel, 7) : '');
  }
  svg += text(0, -5.78, 'WORKSHOP', 10) + text(0, -5.44, '14.435 × 5.406 m', 8);
  for (const area of [plan.lobby, plan.balcony]) {
    const x=(area.minX+area.maxX)/2, z=(area.minZ+area.maxZ)/2;
    svg += text(x,z,area.label.toUpperCase(),8)+text(x,z+.35,area.dimensionLabel,8);
  }
  svg += text(.3, 6.34, 'PRINTER WORKBENCH', 7);
  svg += rect(s.minX, s.topZ, s.maxX - s.minX, s.bottomZ - s.topZ, '#c1cbb3', '#849476');
  const stairLayout = createStairLayout(s, b.room.floorY);
  for (const step of stairLayout.segments) {
    if (step.isLanding) svg += rect(s.minX, step.minZ, s.maxX - s.minX, step.maxZ - step.minZ, '#e1e7d6', '#879778', .6);
    svg += line([s.minX, step.minZ], [s.maxX, step.minZ], '#879778', .6);
    svg += text(s.minX + .27, (step.minZ + step.maxZ) / 2 + .045, String(step.number), 5);
    if (step.isLanding) svg += text((s.minX+s.maxX)/2, (step.minZ+step.maxZ)/2+.1, 'LANDING', 5.5);
  }
  ...
  svg += horizontalDimension(r.minX, r.maxX, -7.7, '5.406 m clear');
  svg += line([-3.38, r.minZ], [-3.38, r.maxZ], '#7d8478', .6) + `<text transform="translate(${X(-3.52)},${Z(0)}) rotate(-90)" text-anchor="middle" font-size="8" fill="#697660">14.435 m clear</text>`;
  svg += '<circle id="map-marker" r="4.5" cx="390" cy="820" fill="#406a40" stroke="white" stroke-width="1.5"/>';
  svg += '<text x="210" y="814" text-anchor="middle" font-size="8" fill="#6e7e65">FIRST FLOOR +3.75 M · PDF WORKSHOP DIMENSIONS</text><text x="210" y="831" text-anchor="middle" font-size="7" fill="#8a9682">Earlier annex layout restored; unlabelled offsets remain estimates.</text></svg>';
  return { svg, X, Z };
}
