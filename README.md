...
The exterior roads follow the owner's confirmed sketch orientation: a broad road at the printer/balcony end, a curved road along the veranda, and a short upper branch to the first-floor lobby. The red-marked strip beside the broad road is also paved, reaching the balcony edge. Bike parking follows the balcony footprint at ground level; three motorcycles face the bamboo-side edge beneath the existing balcony shade. The lower stair access remains connected. Roads share their sampled layout with the reference drawing and collision surfaces; tree placement excludes their footprints. Road widths and curves are indicative because the sketch has no survey dimensions. `node tools/create-site-reference.mjs` regenerates the SVG references in `public/reference`; `node tools/check-site-reference.cjs` exports PNGs and checks live seating, clear aisles and roads. The floor-plan dialog links to the combined reference image.

The workshop TV loads [the live Tinkerspace Calicut website](https://jasimcm.github.io/tinkerspace_digital_calicut/) into an isolated iframe. Its actual scripts, clock and data run from that website; a small document adapter hides its kiosk-only fullscreen prompt and resolves relative assets against the original URL. Fullscreen is disabled for the TV. Every 60 seconds the app fetches the source HTML (the source must allow CORS) and replaces the page only when a new version is available, applying the adapter again. A temporary outage preserves the running page. Internet access is required for the TV. Configure `objects.display.url` and `refreshIntervalMs` in `src/config/scene-config.js`. Press E while facing the TV to view it immediately; R switches power. A CSS3D page behind a depth-tested WebGL screen keeps walls and furniture in front of the TV.
